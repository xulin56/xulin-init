...
@autobind
export default class PullScroll extends React.Component{
    static props = {
    ...
    static defaultProps = {
        dataList : ['列表','列表','列表','列表','列表','列表','列表','列表','列表','列表','列表','列表','列表','列表','列表','列表','列表'],
        upPull : function(){},
        downPull : function(){},
        isFoot : true
    }
    ...
    render(){
        const {dataList,isFoot} = this.props;
        return(
            <div className="pull-scroll">
                <ul onTouchStart={this.touchStart} onTouchEnd={this.touchEnd} ref="onPullUp">
                    {
                ...
                {
                    isFoot ? <span >上拉加载更多</span> : <span>请稍等...</span>
                }
            </div>
        )
    }
